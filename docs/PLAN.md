...
- **Counts toward the allowance**: for an **extra** session, iff `counts_toward_limit` (the teacher's toggle is the whole rule, whatever category colours it); for a **regular** session, iff `counts_toward_limit` **and** its category has a limit. This resolves the brief's silent conflict: a competition-team rehearsal generated from the pattern has `counts_toward_limit = true` by default but its category is unlimited, so it must not eat the two weekly classes the student pays for, while a free extra rehearsal the teacher flips on does count (§9's own example).
...
- **Unenrolled** = per attendance at a regular session whose category the person is not enrolled in on that date. Rows the teacher added by hand (`manual`) are excluded and shown as "added by you" in the evidence instead (question Q9e); walk-ins and auto rows whose enrollment ended are flagged. Extras never raise it.
...
*You can test:* enrol and un-enrol someone and watch future lists change; compose a workshop with a guest and confirm that only listed students can scan into it; review yesterday's seeded class; see a seeded over-user and an unenrolled attendance flagged; an extra's toggle changes the count.
...
| **Q9** | **Overuse rules.** (a) A regular session of an unlimited category (competition team) never consumes the weekly allowance even though it "counts toward limit" by default, while an *extra* counts whenever you flip its toggle on, whatever category it carries; (b) a student with no limited category is never flagged for overuse (only "not enrolled"); (c) attendances in limited categories the student is not enrolled in do count toward the allowance; (d) unreviewed check-ins count until you mark someone absent; (e) a student you added to a class list by hand is not flagged as "not enrolled" (shown as "added by you" instead). | Yes to all five. |
...
| C14 | §9 allowance "of active categories that have a limit" vs "flag when counted attendances in limited categories exceed the allowance" vs §5 `counts_toward_limit` default true for all regular sessions vs §9's example of an extra rehearsal that should count | Undefined with no limited category (allowance 0 would flag everything); a competition rehearsal would eat the weekly classes; an extra tagged with the team's category could never count. | Q9 rules (kind-aware). |
